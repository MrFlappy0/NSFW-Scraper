"""
Xvideos module for NSFW-Scraper.
Extracts video URLs from Xvideos (free content).
"""
...
DESCRIPTION = "Xvideos - Free videos (via yt-dlp)"

async def scrape(url: str) -> List[Dict]:
    """
    Extract video URLs from Xvideos.
    Args:
        url: Xvideos video URL (e.g., https://www.xvideos.com/video123456).
    Returns:
    ...
    print(f"\n🔍 Extracting URLs from Xvideos: {url}")
    ...
    filename = f"xv_{title[:50]}_{media_id}.{ext}"
    target_dir = DOWNLOADS_DIR / "xvideos"
