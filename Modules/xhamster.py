"""
XHamster module for NSFW-Scraper.
Extracts video URLs from XHamster (free content).
"""
...
DESCRIPTION = "XHamster - Free videos (via yt-dlp)"

async def scrape(url: str) -> List[Dict]:
    """
    Extract video URLs from XHamster.
    Args:
        url: XHamster video URL (e.g., https://xhamster.com/videos/abc123).
    Returns:
        List of media dictionaries.
    """
    if not url.startswith("http"):
        url = f"https://xhamster.com/videos/{url}"

    print(f"\n🔍 Extracting URLs from XHamster: {url}")
    ...
    filename = f"xhamster_{title[:50]}_{media_id}.{ext}"
    target_dir = DOWNLOADS_DIR / "xhamster"
